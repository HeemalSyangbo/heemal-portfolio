...
import { achievementSections, capstoneDocs, resourceCategories } from '../data/resources'
...
export function Portfolio() {
  ...
  return (
    <section id="portfolio" className="section-pad relative z-10 scroll-mt-24">
      <SectionHeading
        title="Portfolio"
        subtitle="Résumé, background, capstone documentation, and credentials—quick access for recruiters and hiring managers."
      />

      <div className="mx-auto mt-14 max-w-6xl space-y-20">
        {resourceCategories.map((cat) => (
          <div key={cat.title}>
            <h3 className="mb-8 text-center text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">
              {cat.title}
            </h3>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {cat.items.map((item, i) => {
                const Icon = careerIcons[item.icon] || FileText
                return (
                  <motion.article
                    key={item.id}
                  ...
                  >
                    <Icon className="mb-3 h-6 w-6 text-purple-300" strokeWidth={1.6} />
                    ...
                    {item.action === 'modal' ? (
                      <button
                        ...
                        className="mt-4 rounded-full border border-white/10 bg-white/[0.05] px-3 py-2 text-xs font-semibold text-zinc-200 transition hover:bg-white/[0.09]"
                      >
                        Read
                      </button>
                    ) : item.action === 'anchor' ? (
                      <a
                        href={item.href}
                        className="mt-4 rounded-full border border-white/10 bg-white/[0.05] px-3 py-2 text-center text-xs font-semibold text-zinc-200 transition hover:bg-white/[0.09]"
                      >
                        Open section
                      </a>
                    ) : (
                      <a
                        ...
                        className="mt-4 inline-flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-3 py-2 text-center text-xs font-semibold text-zinc-200 transition hover:bg-white/[0.09]"
                      >
              ...
              <div key={block.id}>
                ...
                <div
                  className={`grid gap-4 ${
                    block.items.length > 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-2'
                  }`}
                >
                  {block.items.map((row, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-3 flex-1 text-sm leading-relaxed text-zinc-400">{row.description}</p>
                        {isPdf ? (
                          <a
                            href={row.href}
                            target="_blank"
        ...
        <div>
          ...
          <p className="mx-auto mb-8 max-w-xl text-center text-sm leading-relaxed text-zinc-400">
            DriveTree (team T06) — formal artifacts from proposal through reports. Open the library for
            all PDFs.
          </p>
          <div className="flex justify-center">
            ...
            >
              ...
              <span className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-purple-500/20 transition group-hover:brightness-110">
                Browse PDFs
      ...
      <Modal open={modal.open} onClose={closeModal} title={modal.title}>
        <p className="whitespace-pre-line">{modal.body}</p>
      </Modal>
